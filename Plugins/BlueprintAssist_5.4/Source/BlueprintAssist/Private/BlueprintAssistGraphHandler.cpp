	...
	if (LinkedToPins.FilterByPredicate(FBAUtils::IsExecPin).Num() == 1)
	{
		...
		if (NodeCreatedOutputExecPins.Num() > 0)
		{
			if (PinOnB->LinkedTo.Num() > 1)
			{
				...
				{
					FBAUtils::TryCreateConnectionUnsafe(ExecPinOnA, NodeCreatedOutputExecPins[0], EBABreakMethod::Always);
				}
			}
		}
	}
	...
	if (LinkedParameterPins.Num() > 0)
	{
		...
		if (PinToLinkTo != nullptr)
		{
			...
			for (UEdGraphPin* Pin : FBAUtils::GetParameterPins(NodeCreated, OtherLinkedPin->Direction))
			{
				...
				{
					bool bConnected = FBAUtils::TryCreateConnectionUnsafe(Pin, PinToLinkTo, EBABreakMethod::Always);
					if (bConnected)
					{
						return;
					}
...
bool FBAGraphHandler::TrySelectFirstPinOnNode(UEdGraphNode* NewNode)
{
	...
	if (FBAUtils::IsNodeImpure(NewNode) && UBASettings_EditorFeatures::Get().PinSelectionMethod_Execution == EBAPinSelectionMethod::Value ||
		FBAUtils::IsNodePure(NewNode) && UBASettings_EditorFeatures::Get().PinSelectionMethod_Parameter == EBAPinSelectionMethod::Value)
	{
